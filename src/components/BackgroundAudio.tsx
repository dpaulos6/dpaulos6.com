...
const BackgroundAudio: React.FC = () => {
  ...
  const [containerSwipeStyles, setContainerSwipeStyles] = useState(
    '-bottom-[4.5rem] group pointer-events-none'
  )
  ...
  return (
    <>
      ...
      <div
        ref={audioContainer}
        className={`fixed min-w-[332px] p-4 xs:w-fit flex flex-col gap-2 right-1/2 translate-x-1/2 md:right-0 md:-translate-x-0 text-text transition-all !duration-500 z-50 ${isTouchDevice ? containerSwipeStyles : '-bottom-[4.5rem] group hover:bottom-0 group'}`}
      >
        <span
          ref={audioLabel}

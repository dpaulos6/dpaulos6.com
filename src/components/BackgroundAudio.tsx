...
const BackgroundAudio: React.FC = () => {
  ...
  return (
    <>
      ...
      <div ref={audioContainer} className={`fixed min-w-[332px] p-4 xs:w-fit flex flex-col gap-2 right-1/2 translate-x-1/2 ${windowWidth && windowWidth <= 768 ? '-bottom-[4.5rem] group' : ''} md:right-0 md:-translate-x-0 text-text transition-all !duration-500 z-50 ${isTouchDevice ? containerSwipeStyles : '-bottom-[4.5rem] group hover:bottom-0 group'}`}>
        <span ref={audioLabel} className={`w-full text-[.85rem] xs:text-sm md:text-base text-center absolute left-1/2 -translate-x-1/2 ${windowWidth && windowWidth <= 768 ? 'top-4 opacity-0' : ''} ${isTouchDevice ? labelSwipeStyles : 'top-4 opacity-0 group-hover:-top-4 group-hover:opacity-100' } transition-all !duration-500 select-none cursor-default`}>
          Click{' '}
